...
SPACESHIP_GIT_OMG_SHOW_UPSTREAM_DEFAULTSAME="${SPACESHIP_GIT_OMG_SHOW_UPSTREAM_DEFAULTSAME=true}" # enhancement to OMG
SPACESHIP_GIT_OMG_DEFAULT_REMOTE="${SPACESHIP_GIT_OMG_DEFAULT_REMOTE="origin"}" # enhancement to OMG
...
custom_build_prompt() {
  ...
  if [[ $SPACESHIP_GIT_OMG_HIDE_FILE_COUNTS == false ]]; then
    local number_of_modified_files=${24}
    local number_of_modified_cached_files=${25}
    local number_of_added_files=${26}
    local number_of_deleted_files=${27}
    local number_of_deleted_cached_files=${28}
    local number_of_ready_to_commit_files=${29}
    local number_of_untracked_files=${30}
    local number_of_stashes=${31}
  fi
    ...
    if [[ $detached == true ]]; then
      ...
      if [[ $has_upstream == false ]]; then
        ...
        if [[ $SPACESHIP_GIT_OMG_SHOW_UPSTREAM == true ]]; then
          if [[ $SPACESHIP_GIT_OMG_SHOW_UPSTREAM_DEFAULTSAME == true && "$upstream" == "${SPACESHIP_GIT_OMG_DEFAULT_REMOTE}/${current_branch}" ]]; then
            omg_branch="${SPACESHIP_GIT_OMG_WHERE_BRANCH_PREFIX}${display_branch} ${type_of_upstream}${SPACESHIP_GIT_OMG_DEFAULT_REMOTE}${SPACESHIP_GIT_OMG_WHERE_BRANCH_SUFFIX}"
          elif [[ "$upstream" != "${SPACESHIP_GIT_OMG_DEFAULT_REMOTE}/${current_branch}" ]]; then
